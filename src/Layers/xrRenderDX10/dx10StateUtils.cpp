...
namespace dx10StateUtils
{
	...
	bool operator==(const D3D_BLEND_DESC& desc1, const D3D_BLEND_DESC& desc2)
	{
		...
		for (int i = 0; i < 4/*8*//*It's quick fix for dx11 port because of mispatch of desc*/; ++i)
		{
			...
			if (desc1.RenderTarget[i].RenderTargetWriteMask != desc2.RenderTarget[i].RenderTargetWriteMask) return
				false;
		}
	...
	u32 GetHash(const D3D_BLEND_DESC& desc)
	{
		...
		for (int i = 0; i < 8; ++i)
		{
			...
			Hash.AddData(&desc.RenderTarget[i].RenderTargetWriteMask,
			             sizeof(desc.RenderTarget[i].RenderTargetWriteMask));
		}
