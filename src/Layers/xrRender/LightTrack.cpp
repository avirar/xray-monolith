...
void CROS_impl::update(IRenderable* O)
{
	...
	if (MODE & IRender_ObjectSpecific::TRACE_HEMI) hemi.mul(hemi_smooth);
	else hemi.mul(.2f);
	accum.add(hemi);
	if (MODE & IRender_ObjectSpecific::TRACE_SUN) sun_.mul(sun_smooth);
	else sun_.mul(.2f);
	accum.add(sun_);
	if (MODE & IRender_ObjectSpecific::TRACE_LIGHTS)
	{
		...
		for (size_t i = 0; i < NUM_FACES; ++i)
		{
			hemi_cube[i] += hemi_cube_light[i] * (1 - ps_r2_dhemi_light_flow) + ps_r2_dhemi_light_flow * hemi_cube_light
				[(i + NUM_FACES / 2) % NUM_FACES];
...
void CROS_impl::calc_sun_value(Fvector& position, CObject* _object)
{
	...
	if (MODE & IRender_ObjectSpecific::TRACE_SUN)
	{
		if (--result_sun < 0)
		{
			...
			sun_value = !(g_pGameLevel->ObjectSpace.RayTest(position, direction, 500.f, collide::rqtBoth, &cache_sun,
			                                                _object))
				            ? 1.f
				            : 0.f;
		}
