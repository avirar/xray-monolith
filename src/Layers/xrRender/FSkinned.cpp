...
#if !defined(USE_DX10) && !defined(USE_DX11) && !defined(USE_DX12) && !defined(USE_DX12)	//	Don't use hardware buffers in DX10 since we can't read them
	switch (RenderMode)
	{
	case RM_SKINNING_SOFT:
#endif	//	USE_DX10
                               ...
                               Fvisual* V, u16 bone_id, u32 iBase, u32 iCount)
{
	...
	case RM_SKINNING_1B: result = _PickBoneHW1W(r, dist, start, dir, V, indices + iBase, *faces);
		break;
	case RM_SKINNING_2B: result = _PickBoneHW2W(r, dist, start, dir, V, indices + iBase, *faces);
		break;
	case RM_SKINNING_3B: result = _PickBoneHW3W(r, dist, start, dir, V, indices + iBase, *faces);
		break;
	case RM_SKINNING_4B: result = _PickBoneHW4W(r, dist, start, dir, V, indices + iBase, *faces);
		break;
	default: NODEFAULT;
	}
...
BOOL CSkeletonX_ST::PickBone(IKinematics::pick_result& r, float dist, const Fvector& start, const Fvector& dir,
                             u16 bone_id)
{
	return inherited2::_PickBone(r, dist, start, dir, this, bone_id, iBase, iCount);
}

BOOL CSkeletonX_PM::PickBone(IKinematics::pick_result& r, float dist, const Fvector& start, const Fvector& dir,
                             u16 bone_id)
{
...
void CSkeletonX_ST::FillVertices(const Fmatrix& view, CSkeletonWallmark& wm, const Fvector& normal, float size,
                                 u16 bone_id)
{
	inherited2::_FillVertices(view, wm, normal, size, this, bone_id, iBase, iCount);
}

void CSkeletonX_PM::FillVertices(const Fmatrix& view, CSkeletonWallmark& wm, const Fvector& normal, float size,
                                 u16 bone_id)
{
	FSlideWindow& SW = nSWI.sw[0];
...
void CSkeletonX_ext::_EnumBoneVertices(SEnumVerticesCallback& C, Fvisual* V, u16 bone_id, u32 iBase, u32 iCount) const
{
	...
	case RM_SKINNING_1B: TEnumBoneVertices((vertHW_1W*)vertices, indices + iBase, *faces, C);
		break;
	case RM_SKINNING_2B: TEnumBoneVertices((vertHW_2W*)vertices, indices + iBase, *faces, C);
		break;
	case RM_SKINNING_3B: TEnumBoneVertices((vertHW_3W*)vertices, indices + iBase, *faces, C);
		break;
	case RM_SKINNING_4B: TEnumBoneVertices((vertHW_4W*)vertices, indices + iBase, *faces, C);
		break;
	default: NODEFAULT;
	}
	if (RenderMode != RM_SKINNING_SOFT)
